...
local function MoveFrame(name, anchor, offsetX, offsetY)
    local frame = name
...
local function SetButtonProperties(button, normalTexture, highlightTexture, pushedTexture, width, height, pointA, pointB,
    ...
    if button == QuestFramePushQuestButton then
...
-- SetFontColor(QuestLogObjectiveText, "DarkModeGrey70")
...
function QuestLogButtons()

-- ShareQuest Button
    SetButtonProperties(QuestFramePushQuestButton, texturePath,
        "Interface\\AddOns\\DialogUI\\UI\\ButtonHighlight-Gossip",
        texturePath,
        100, 30, "BOTTOMLEFT", "BOTTOMLEFT", 80, 50, "DarkModeGold")
		
		-- Adjust texture coordinates to maintain aspect ratio
		--SetTexCoord(0, 1, 0, 1)
		QuestFramePushQuestButton:GetNormalTexture():SetAllPoints(QuestFramePushQuestButton)
		QuestFramePushQuestButton:GetHighlightTexture():SetAllPoints(QuestFramePushQuestButton)
		QuestFramePushQuestButton:GetPushedTexture():SetAllPoints(QuestFramePushQuestButton)


-- Abandon Quest Button
    SetButtonProperties(QuestLogFrameAbandonButton, texturePath,
        "Interface\\AddOns\\DialogUI\\UI\\ButtonHighlight-Gossip",
        texturePath,
        100, 30, "BOTTOMLEFT", "BOTTOMLEFT", 80, 50, "DarkModeGold")
		...
		-- Adjust texture coordinates to maintain aspect ratio
		--SetTexCoord(0, 1, 0, 1)
		QuestLogFrameAbandonButton:GetNormalTexture():SetAllPoints(QuestLogFrameAbandonButton)
		QuestLogFrameAbandonButton:GetHighlightTexture():SetAllPoints(QuestLogFrameAbandonButton)
		QuestLogFrameAbandonButton:GetPushedTexture():SetAllPoints(QuestLogFrameAbandonButton)

end



QuestLogFrame:SetScript("OnShow",
    function(self)
		QuestLogFrame:SetScale(1.2)
...
function QuestLog_UpdateQuestDetails(doNotScroll)
	...
	for i=1, numObjectives, 1 do
		...
		if ( finished ) then
			string:SetTextColor(1, 0.98, 0.8);
			text = text.." ("..TEXT(COMPLETE)..")";
		else
			string:SetTextColor(0.796, 0.784, 0.584);
		end
		string:SetText(text);
		string:Show();
